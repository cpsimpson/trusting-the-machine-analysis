...
pal_2 = c("#994a6f", "#6f994a", "#4a6f99", "#267843", "grey40", "#f4849f")
pal_3 = c("#c485a2", "#a2c485", "#85a2c4")
pal_4 = c("#7F254390", "#26784390","#19638990", "#2f258990",  "grey40", "#f4849f")
pal_4 = c("#7F254375", "#26784375","#19638975", "#2f258990",  "grey40", "#f4849f")
fill_pal = c("#7F254350",
             "#19638950",
             "#2f258950",
             "#26784350",
...
violin_plot <- function(data,
  ...
  ggsave(paste(sep = "/", "plots", study, filename),
         plot = plot,
         create.dir = TRUE)
...
correlation_plot <- function(data, study, x_col_name, y_col_name, save = TRUE) {
  legend_position = "none"
  x = data |> pull({{ x_col_name }})
  y = data |> pull({{ y_col_name }})
  x_label = get_label(x_col_name)
...
categorical_interaction_plot_3 <- function(data,
                                           lm_model,
                                           pred_name,
                                           mod_name,
                                           target_name,
...
interaction_plot_3 <- function(data,
                               lm_model,
                               pred_name,
                               mod_name,
                               target_name,
...
interaction_plot_4 <- function(data,
                               lm_model,
                               pred_name,
                               mod_name,
                               mod2_name,
...
node_colors <- c(
  ...
  # Cnd (Condition)
  "#f4849f",
  # AIC (Chatbot Frequency)
  "#f4849f",
  # A_6 (Appelman 6)
  "#f4849f",
  # SC_ (Science Content Frequency)
...
plot_lavaan_model <- function(fit, labels, name, study, subset = NULL) {
  
  if (!requireNamespace("DiagrammeRsvg", quietly = TRUE)) stop("Install 'DiagrammeRsvg'.")
  if (!requireNamespace("rsvg", quietly = TRUE)) stop("Install 'rsvg'.")
  if (!requireNamespace("lavaanPlot", quietly = TRUE)) stop("Install 'lavaanPlot'.")
  
  plot <- lavaanPlot::lavaanPlot(
    fit,
    labels = labels,
    coefs = TRUE,
    # sig = .05,
  ...
  filename <- paste0("sem_", name, ".png")
  path <- paste(sep = "/", "plots", study, filename)
  
  lavaanPlot::save_png(plot, path, width = 1024, height = 1024)
