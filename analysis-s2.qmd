...
raw_data <- read_survey(
  "data/s2/Thesis+-+Trusting+the+Machine+-+Interactive_March+6,+2025_20.24.csv", 
  col_types = readr::cols(AIChatbotsUsed = readr::col_character())
)

cols <- c(SurveyTopicCheck_coded = runif(n=1, min=0, max=5), Unrealistic_coded = runif(n=1, min=0, max=5))
raw_data <- raw_data %>% 
...
```{r}
meaned_data <- raw_data %>% mutate(across(where(is.numeric), ~replace_na(., mean(., na.rm=TRUE))))
...
cleaned_data <- recoded_data |>
  ...
  filter(id != 472759) # didn't read the blog post and didn't get answers from bot
...
Number of participants after basic filtering = `r nrow(cleaned_data)`.
...
fit_i <- lm(content_trust_combined_score ~ anthropomorphism_score , data = cleaned_data)
...
#TODO: put this back when coding is complete
...
fit_i <- lm(author_trust_combined_score ~ anthropomorphism_score , data = cleaned_data)
