...
raw_data <- read_survey(
  "data/s2/Thesis+-+Trusting+the+Machine+-+Interactive_March+3,+2025_20.27.csv", 
  col_types = readr::cols(AIChatbotsUsed = readr::col_character())
)

cols <- c(SurveyTopicCheck_coded = 0, Unrealistic_coded = 0)
raw_data <- raw_data %>% 
...
cleaned_data <- recoded_data |>
  ...
  filter(ConsentForm == 1) |>  # Provided consent
  filter(Status == 0) |> # Used anonymous link to complete survey (not preview)
  filter(`Duration (in seconds)` >= (mean(`Duration (in seconds)`)) / 3) |>
