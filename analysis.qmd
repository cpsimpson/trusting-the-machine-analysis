...
compare_means(content_trust_combined_score ~ AIChatbotsFrequency_regrouped, comparisons = my_comparisons, data = cleaned_data,  method = "t.test")
...
p <- ggplot(df_summary, aes(x = AIChatbotsFrequency_regrouped, 
                            y = mean_score, 
                            fill = AIChatbotsFrequency_regrouped)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), color = "black") +
  geom_errorbar(aes(ymin = mean_score - sd_score, ymax = mean_score + sd_score), 
                position = position_dodge(width = 0.8), width = 0.2) +
  labs(
    x = "AI Usage Frequency",
    y = "Mean Content Trust Score",
    fill = "AI Usage Frequency"
  ) +
  theme_minimal() +
  theme(legend.position = "top") +
  scale_fill_brewer(palette = "Set1") +
  ggtitle("Interaction of AI Usage Frequency on Content Trust")

p <- p + stat_compare_means(
  aes(group = AIChatbotsFrequency_regrouped),
  method = "t.test",  # Or use "wilcox.test" for non-parametric data
  comparisons = list(
    c("rarely", "occassionally"),
    c("rarely", "frequently"),
    c("occassionally", "frequently")
  ),
  label = "p.signif",
  label.y = max(df_summary$mean_score + df_summary$sd_score) * 1.1
)
...
```{r}
aov_model <- aov(content_trust_combined_score ~ AIChatbotsFrequency_regrouped, data = cleaned_data)
summary(aov_model)

pairwise.t.test(cleaned_data$content_trust_combined_score, cleaned_data$AIChatbotsFrequency_regrouped,  p.adjust.method = "bonferroni")

# Test assumptions for the ANOVA

# Test for Normality
hist(cleaned_data$content_trust_combined_score)

qqnorm(aov_model$residuals)
qqline(aov_model$residuals)

shapiro.test(cleaned_data$content_trust_combined_score)

# Test for Equal Variance
boxplot(content_trust_combined_score ~ AIChatbotsFrequency_regrouped, xlab='AI Usage Frequency', ylab='Content Trust', data=cleaned_data)
bartlett.test(content_trust_combined_score ~ AIChatbotsFrequency_regrouped, data=cleaned_data)

kt_model <- kruskal.test(content_trust_combined_score ~ AIChatbotsFrequency_regrouped, data = cleaned_data) 

summary(kt_model)

dunnTest(content_trust_combined_score ~ AIChatbotsFrequency_regrouped,
         data=cleaned_data,
         method="bonferroni")
