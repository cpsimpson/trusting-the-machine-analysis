...
shapiro_results <- raw_data %>%
  select(-ConsentForm, -`Duration (in seconds)`) %>%
...
data <- cleaned_data
formula <- anthropomorphism_score ~ Condition
x <- cleaned_data$Condition
x_label <- "Condition"
y <- cleaned_data$anthropomorphism_score
y_label <- "Anthropomorphism Score"
...
```{r}
cor_result <- cor.test(cleaned_data$content_trust_combined_score, cleaned_data$Age_1, method = "pearson")
apa::cor_apa(cor_result, r_ci = TRUE)

cleaned_data |>
  ggplot(aes(x = Age_1, y = content_trust_combined_score)) +
  geom_point() +
        stat_smooth(method = "lm",
        col = "#C42126", se = FALSE, size = 1
)
