...
-   How much violation of assumptions for ANOVA is too much?

    -   Should I be switching to non-parametric tests? (I ran a few here as a sample, since this data does violate the assumptions)

    -   Is there a different adjustment to the data that would be better?

    -   I could use some help interpreting some of the linear model data for interactions. For example, see Age under Content Trust and Author Trust analysis. There are a few pairings that have significant p-values, but I'm having trouble seeing if it means anything. Which kind of makes me think it isn't really meaningful...

-   Is there a good way to check whether the variance in correlations across conditions is significant? Is this just done by doing an ANOVA?

-   I don't really understand what is going on with Author Trust by Frequency of AI Chatbot usage, it seems like there is an effect at all Condition levels, in the same directions, but no overall effect. How is that possible? (maybe I did something wrong...) Maybe it is because of the outliers that are spread across all the conditions?

## Still To Do:

-   Fix the "Changed Opinion" section - I think it should use interaction_plot instead of cat_plot

-   Fix the spelling of occasionally

-   Check the significance of correlation differences across conditions.
...
frequency_comparisons <- list(  c("rarely", "occassionally"),
    c("rarely", "frequently"),
...
ggbarplot(cleaned_data |> filter(AIChatbotsFrequency_regrouped == "occassionally"), x = "Condition", y = "content_trust_combined_score",
          add = "mean_sd",        
          color = "Condition", palette = safe_pal, position = position_dodge(0.8))+ 
  stat_compare_means(comparisons = condition_comparisons, method = "t.test", label = "p.signif") +
  stat_compare_means(label.y = 12, method = "anova")  +     # Add global p-value
...
ggbarplot(cleaned_data |> filter(AIChatbotsFrequency_regrouped == "occassionally"), x = "Condition", y = "author_trust_combined_score",
          add = "mean_sd",        
          color = "Condition", palette = safe_pal, position = position_dodge(0.8))+ 
  stat_compare_means(comparisons = condition_comparisons, method = "t.test", label = "p.signif") +
  stat_compare_means(label.y = 12, method = "anova")  +     # Add global p-value
